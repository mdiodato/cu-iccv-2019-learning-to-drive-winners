...
# required.
config = json.load(open(save_dir + 'config_2.json'))
...
# The model that takes uses resnet152 and a simple 2 layer NN + LSTM architecture to predict canSteering and canSpeed. 
class SomeDrivingModel(nn.Module):
    def __init__(self):
        ...
        if config['front']: #CNN for front images
            cnn_front = models.resnet152(pretrained=True)
        ...
        if config['multi_camera']['right_left']: #CNN for side images
            cnn_ls = models.resnet152(pretrained=True)
            ...
            cnn_rs = models.resnet152(pretrained=True)
        ...
        if config['multi_camera']['rear']: #CNN for rear images
            cnn_back = models.resnet152(pretrained=True)
